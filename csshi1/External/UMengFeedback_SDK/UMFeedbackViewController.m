...
//    frame.origin.y = self.topViewOffset + originY;//自己改动
...
//    frame.origin.y += _navButtonHeight;//自己改动
...
- (void)loadView {
    ...
    self.mTableView.backgroundColor =  M1905_UIColorFromRGB(0xf2f2f2);
...
//        cell.messageBackgroundView = [[UIImageView alloc] init];//WithImage:[UIImage imageNamed:@"fankui_yonghu_bg_normal"]];//self.outgoingBubbleImageView.image
//                                                       highlightedImage:self.outgoingBubbleImageView.highlightedImage];
    } else {
        cell.isRightAlign = NO;
//        cell.messageBackgroundView = [[UIImageView alloc] init];//WithImage:[UIImage imageNamed:@"fankui_kefu_bg_normal"]];
//                                      self.incomingBubbleImageView.image
...
- (CGFloat)tableView:(UITableView *)tableView heightForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if (content.length > 0) {
        CGSize labelSize = [content sizeWithFont:[UIFont systemFontOfSize:14.0f]
                               constrainedToSize:CGSizeMake(self.mTableView.frame.size.width - 100,MAXFLOAT)
